...
- [Dev](#dev): write your application as a set of ordinary Clojure functions
- [Push](#push): capture your current local view of the world as a *revision* you can later deploy
- [Deploy](#deploy): a revision to a Datomic compute group
- [Bond](#bond): use the functions you deployed in transactions, queries, AWS Lambdas, or web services
...
Ions include one or more [entry points](../02-ions-reference/ions-reference.md#ion-entry-points). These are Clojure functions that have a signature matching their intended use:
...
- Web service functions support [ring-like edn handlers](../02-ions-reference/ions-reference.md#http-direct-entry-point)
...
When you create a [primary compute group](../../05-operation/02-cloud/01-cloud-architecture/cloud-architecture.md#compute-groups) or query group, you associate it with a [Code Deploy](https://aws.amazon.com/codedeploy/) application. The `:app-name` in your ion-config.edn connects your code to a Code Deploy application and determines which compute groups you can deploy to.
...
- As [transaction functions](../../06-reference/02-transactions/04-transaction-functions/transaction-functions.md#writing-transaction-functions) inside a Datomic transaction
...
- As [lambda functions](../02-ions-reference/ions-reference.md#invoking-lambdas), either directly or in response to AWS events
- As web services via AWS API Gateway and either [HTTP direct](../02-ions-reference/ions-reference.md#http-direct-configuration) or a [lambda proxy](../02-ions-reference/ions-reference.md#lambdas)
