...
The Datomic Peer API [names databases](../../../04-apis/01-peer-api-clojuredoc/peer-api-clojuredoc.md#connect) with a URI that includes the protocol name, storage connection information, and a database name. The complete URI for a database named "hello" on the [transactor you started in the previous step](../01-run-a-transactor/run-a-transactor.md#starting-a-transactor) is `"datomic:dev://localhost:4334/hello"`.
...
Once you have [created a database](#creating-a-database), you can use the Datomic peer library to interact with it.
