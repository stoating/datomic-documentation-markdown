...
NOTE: Read [deployment](../../05-operation/01-pro/03-datomic-deployment/datomic-deployment.md) before calling.
...
See [pull](../../06-reference/03-query-and-pull/03-pull/pull.md) for more information.
...
See [query data reference: find specs](../../06-reference/03-query-and-pull/02-query-reference/query-reference.md#find-specs).
...
error handling, see [error handling](../13-error-handling/error-handling.md).
Note that an exception may occur either when invoking the API
or when dereferencing the returned reference.

See [io-stats](../10-io-stats/io-stats.md) for how
...
See [transaction data reference](../../06-reference/02-transactions/02-transaction-data/transaction-data.md).
...
- `:io-context` — a qualified keyword; when provided, the return map will have an additional `:io-stats` key describing information about the I/O reads performed by the call to `d/with`. See [io-stats](../10-io-stats/io-stats.md).
- `:return-hints true` — when provided, the return map may also include an additional key `:hints`, an opaque value that can be passed as an option to `d/transact` or `d/transact-async` to optimize performance. See [reducing latency with transaction hints](../../06-reference/02-transactions/08-reducing-latency-with-transaction-hints/reducing-latency-with-transaction-hints.md).
