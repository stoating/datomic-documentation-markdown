...
Every Datomic node publishes a JvmFreeMb [metric](../12-monitoring-cloud/monitoring-cloud.md#metrics). If the Minimum value for this metric dips below 15% of its initial startup value for an extended period, memory pressure is likely a problem. Consider the following:
...
When troubleshooting a Solo system, ensure that the single compute node in your system is healthy and reporting [logs](../12-monitoring-cloud/monitoring-cloud.md#logs) and [metrics](../12-monitoring-cloud/monitoring-cloud.md#metrics). If it is not, you should use the [EC2 dashboard](https://console.aws.amazon.com/ec2/) to terminate your compute node and allow the AutoScaling group to replace it.
...
> Information for previous CLI tools: please update to [the latest version](../../../11-releases/releases.md#current).
...
- Provide an [unreproducible name](../../../07-datomic-cloud-ions/02-ions-reference/ions-reference.md#unreproducible)
