...
Peers can continue to read and query during a transactor failover. In particular, calls to [datomic.api/db](../../../04-apis/01-peer-api-clojuredoc/peer-api-clojuredoc.md#db) will neither block nor fail during the window of a transactor failover.
...
Datomic does not automatically retry transactions after a transactor failure. Applications, aware of their own semantic needs, can implement an appropriate retry policy, with full access (e.g. via the [Log API](../../../04-apis/08-log-api/log-api.md)) to recent transaction history.
...
Datomic's [backup](../07-backup-and-restore/backup-and-restore.md) makes a *consistent copy* of Datomic data, i.e. backup preserves Datomic's consistency guarantees. Thus Datomic backup is suitable for use in a disaster-recovery plan.
